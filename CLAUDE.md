...
- The association's official logo files are `media-source/ProKuolimologo_*.png`
  (white for the dark site, dark for print). `build-images.sh` trims their
  transparent margin into `assets/img/pro-kuolimo-logo-{valkoinen,tumma}.png`
  and does nothing else — no recolouring, no redrawing. The trim is not
  cosmetic: nearly half of each file's height is empty, so used untrimmed the
  topbar would grow from 64px to about 106px. The same image serves the topbar
  (2.75rem tall) and the footer (19rem wide); print swaps in the dark one,
  because the white logo would be invisible on paper. An earlier version of this
  logo was reconstructed from a PDF header — that is gone, and these files are
  the source of truth.
