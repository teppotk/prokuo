...
python3 tools/pura-logo.py                  # regenerate assets/img/pro-kuolimo-logo.png
```
...
- The association's real logo is in the footer, generated by `tools/pura-logo.py`
  from the header of `aineistot/2025-11-19-nakosyvyysmittausten-ohjeistus.pdf` —
  there is no separate logo file anywhere. It is two flat colours over white
  (text `#222222`, waves `#2E91C9`), so the tool unpicks the anti-aliasing back
  into an alpha channel instead of leaving a white plate, and recolours the text
  to `--ink` for the dark background. It is the same artwork reversed, not a
